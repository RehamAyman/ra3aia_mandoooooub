...
class ChatPrisnter {
    ...
    func SockeConFigration(){
        SocketConnection.sharedInstance.socket.on("newMessage") { (data, ack) in
            ...
            self.MessagesArray.append(chatMessages(id: 0, content: dict["content"] as? String ?? "", avatar: User.currentUser?.avatar ?? "", type: dict["type"] as? String ?? "" ,created_at: "", sent_by_me: 0))
    ...
    func sendMessages(message:String,type:String){
        if(message != ""){
            ...
            self.MessagesArray.append(chatMessages(id: 0, content: message, avatar: User.currentUser?.avatar ?? "", type: type ,created_at: "", sent_by_me: 1))
